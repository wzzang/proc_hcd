...
# set rs-fMRI dir keys
    # 8 possible combos for atypical REST scans: 
        # 1a_AP, 1b_AP, 1a_PA, 1b_PA
        # 2a_AP, 2b_AP, 2a_PA, 2b_PA
dir_keys=( rfMRI_REST1a_AP rfMRI_REST1a_PA rfMRI_REST2a_AP rfMRI_REST2a_PA \
rfMRI_REST1b_AP rfMRI_REST1b_PA rfMRI_REST2b_AP rfMRI_REST2b_PA \
T1w_MPR_vNav_4e_e1e2_mean T2w_SPC_vNav )
# get containers
    ...
    for d in ${dir_keys[@]} ; do        
        if [[ -d $d ]] ; then
            if echo "$d" | grep -q "REST"; then
                ...
                r=`echo $d | awk -F"REST"  '{print $2}' | cut -c1-2` # run number, eg, 1b
                
                # recode run number for bidsification
                # 1a -> 011; 1b -> 012; 2a -> 021; 2b -> 022
                if [ "$r" == '1a' ] ; then
                    new_r=011
                elif [ "$r" == '1b' ] ; then
                    new_r=012
                ...
                # rfMRI
                bs_rest=sub-${sub_id}_ses-01_task-rest_dir-${k}_run-${new_r}_bold
                bs_sbr=sub-${sub_id}_ses-01_task-rest_dir-${k}_run-${new_r}_sbref
                bs_fmap=sub-${sub_id}_ses-01_dir-${k}_run-${new_r}_epi
...
if [ ! -f ${json_job} ] ; then
    echo ".. JSON modification" >> ${sub_log}
    for json in ${sub_dir}/fmap/*.json ; do
        # echo $json
        if [[ "$json" == *"AP_run-011"* ]]; then
            # echo $json
            jq --arg sub "sub-${sub_id}" '. + { "IntendedFor": [
                "ses-01/func/\($sub)_ses-01_task-rest_dir-AP_run-011_bold.nii.gz"
            ] }' "$json" > ${sub_dir}/fmap/tmp.json && mv ${sub_dir}/fmap/tmp.json $json
        elif [[ "$json" == *"PA_run-011"* ]]; then
            jq --arg sub "sub-${sub_id}" '. + { "IntendedFor": [
                "ses-01/func/\($sub)_ses-01_task-rest_dir-PA_run-011_bold.nii.gz"
            ] }' "$json" > ${sub_dir}/fmap/tmp.json && mv ${sub_dir}/fmap/tmp.json $json
        elif [[ "$json" == *"AP_run-012"* ]]; then
            jq --arg sub "sub-${sub_id}" '. + { "IntendedFor": [
                "ses-01/func/\($sub)_ses-01_task-rest_dir-AP_run-012_bold.nii.gz"
            ] }' "$json" > ${sub_dir}/fmap/tmp.json && mv ${sub_dir}/fmap/tmp.json $json
        elif [[ "$json" == *"PA_run-012"* ]]; then
            jq --arg sub "sub-${sub_id}" '. + { "IntendedFor": [
                "ses-01/func/\($sub)_ses-01_task-rest_dir-PA_run-012_bold.nii.gz"
            ] }' "$json" > ${sub_dir}/fmap/tmp.json && mv ${sub_dir}/fmap/tmp.json $json
        elif [[ "$json" == *"AP_run-021"* ]]; then
            jq --arg sub "sub-${sub_id}" '. + { "IntendedFor": [
                "ses-01/func/\($sub)_ses-01_task-rest_dir-AP_run-021_bold.nii.gz"
            ] }' "$json" > ${sub_dir}/fmap/tmp.json && mv ${sub_dir}/fmap/tmp.json $json
        elif [[ "$json" == *"PA_run-021"* ]]; then
            jq --arg sub "sub-${sub_id}" '. + { "IntendedFor": [
                "ses-01/func/\($sub)_ses-01_task-rest_dir-PA_run-021_bold.nii.gz"
            ] }' "$json" > ${sub_dir}/fmap/tmp.json && mv ${sub_dir}/fmap/tmp.json $json
        elif [[ "$json" == *"AP_run-022"* ]]; then
            jq --arg sub "sub-${sub_id}" '. + { "IntendedFor": [
                "ses-01/func/\($sub)_ses-01_task-rest_dir-AP_run-022_bold.nii.gz"
            ] }' "$json" > ${sub_dir}/fmap/tmp.json && mv ${sub_dir}/fmap/tmp.json $json
        elif [[ "$json" == *"PA_run-022"* ]]; then
            jq --arg sub "sub-${sub_id}" '. + { "IntendedFor": [
                "ses-01/func/\($sub)_ses-01_task-rest_dir-PA_run-022_bold.nii.gz"
            ] }' "$json" > ${sub_dir}/fmap/tmp.json && mv ${sub_dir}/fmap/tmp.json $json
        fi      
    done
    # save log
